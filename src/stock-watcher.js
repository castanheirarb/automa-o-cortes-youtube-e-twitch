...
//   • Trend Hunter                  → captureTrendClip (corte em alta)
//   • Vídeos longos (output/longos) → replicateTrendingLongVideo (vídeo inteiro)
//   • Vídeo longo Canal da Fé (output/longos-fe) → getBispoLongVideo (prédica/live inteira)
//
// .env:
...
async function main() {
    ...
    // ── Vídeo longo do Canal da Fé (sempre do próprio canal do Bispo) ────────
    if (process.env.LONG_VIDEO_FE_ENABLED !== 'false') {
        ...
        if (count < MIN_LONG_FE) {
            logger.step(`[Stock] Vídeo longo do Canal da Fé com ${count}/${MIN_LONG_FE} — baixando prédica do Bispo...`);
            try {
                const { getBispoLongVideo } = await import('./canal-da-fe/long-video.js');
                await getBispoLongVideo();
                reposicoes++;
            } catch (err) {
